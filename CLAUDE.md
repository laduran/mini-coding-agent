...
1. **Prefix / prompt assembly** (`build_prefix`, `prompt`): a stable instruction+tool-schema prefix (built once at construction, cache-friendly) is concatenated each turn with distilled memory, materialized file views, clipped transcript history, and the current user message. There is no chat/message-array API — the whole thing is one text blob POSTed to Ollama's `/api/generate`.
...
4. **Loop control** (`ask`): iterates up to `max_steps` tool calls (with a separate, larger `max_attempts` budget that also counts malformed-output retries) until the model returns `<final>`. `repeated_tool_call()` rejects a tool call that's identical (args normalized against `TOOL_ARG_DEFAULTS`) to either of the last two tool calls, to break the model out of stall loops — **except** a repeated `read_file` whose contents are no longer materialized (see below), where blocking would strand the model. Rejections are built by `repeat_rejection()` and prefixed with the module-level `REPEAT_REJECTION` constant; `ask()` keys off that prefix to emit a distinct `stalled: ... (Nx in a row)` progress line, so going in circles reads differently from an ordinary tool failure.
5. **File views** (`touched_paths`, `file_views`, `view_for`, `file_views_text`): every file the model has read or written is re-read fresh from disk each turn and rendered in a dedicated `Files you have read:` prompt section, budgeted by `MAX_FILE_VIEWS` / `MAX_FILE_VIEW_CHARS` / `MAX_FILE_VIEWS_TOTAL` (evicting least-recently-touched first). This exists because recency clipping used to erode file contents out of the prompt while the loop guard simultaneously forbade re-reading them — a deadlock. Reading from disk also means a view can never go stale after a write. Views are the authority for "does the model still have this file?", which is what the guard consults.
6. **Memory & transcript** (`note_tool`, `memory_text`, `history_text`, `record`): every turn is appended to `session["history"]` and persisted immediately via `SessionStore.save`. `session["memory"]` (task/files/notes, each capped via `remember()`) is a small distilled summary threaded into every prompt so context survives beyond the clipped transcript. In `history_text()`, a `read_file` result whose path is materialized collapses to a one-line pointer at the Files section (no second clipped copy); paths that are *not* materialized fall back to the older dedup rule (repeat reads dropped unless a write intervened), and recent turns (last 6) get a bigger clip budget than older ones.
7. **Delegation** (`tool_delegate`): spawns a child `MiniAgent` at `depth+1`, `read_only=True`, `approval_policy="never"`, sharing the same model client/workspace/session_store — bounded (`max_depth`, default 1) so delegation can't recurse indefinitely.
